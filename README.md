...
* [`src/clj/clj_test/`](src/clj/clj_test/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/clj_test/`](src/cljs/clj_test/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/clj_test/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `clojure-projects` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clj_test/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure-projects.example
  (:require [clojure-projects.config :as config])
...
standalone `clojure-projects.jar`.
...
```sh
port=2000 java -jar target/clojure-projects.jar
```
